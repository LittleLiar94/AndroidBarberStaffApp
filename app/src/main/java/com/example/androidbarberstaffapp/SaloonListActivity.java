...
import com.example.androidbarberstaffapp.Adapter.MySaloonAdapter;
...
import com.example.androidbarberstaffapp.Interface.IOnLoadCountSaloon;
...
public class SaloonListActivity extends AppCompatActivity implements IOnLoadCountSaloon, IBranchLoadListener, IGetBarberListener, IUserLoginRememberListener {
    ...
    private void loadSaloonBasedOnCity(String name) {
        dialog.show();
        FirebaseFirestore.getInstance().collection("AllSaloon")
                .document(name)
                .collection("Branch")
                .get()
    ...
    public void onLoadCountSaloonSuccess(int count) {
        txt_saloon_count.setText(new StringBuilder("All Saloon(")
        .append(count)
        .append(")"));
